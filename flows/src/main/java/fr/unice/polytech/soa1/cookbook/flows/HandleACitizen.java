...
public class HandleACitizen extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from(HANDLE_CITIZEN)
				...
				.choice()
					.when(simple("${body.quantity} >= 42000"))
						.setProperty("tax_computation_method", constant("COMPLEX"))
						.to("direct:complexTaxMethod")
					.when(simple("${body.price} >= 0 && ${body.price} < 42000"))
						.setProperty("tax_computation_method", constant("COMMAND"))
		...
		from("direct:generateLetter")
				...
				.to(CSV_OUTPUT_DIRECTORY + "?fileName=file.txt")//"?fileName=${property.p_uuid}.txt")
